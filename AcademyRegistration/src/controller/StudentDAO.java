...
public class StudentDAO {
	...
	public String getStudentCount(String subejectNum) throws Exception {
		String sql = "select LPAD(count(*)+1, 4,'0') as studentCount from student where s_num = ?";// LPAD(값,총
																									// 문자의길이,채울문자)
	...
	public String getStudentNum(String id, String pw) throws Exception {
		String sql = "select sd_num from student where sd_id = ? and sd_passwdF= ?";
	...
	public void getStudentTotalList() throws Exception {
		String sql = "select st.no as no, sd_num, sd_name, sd_id, sd_passwd,su.s_name as s_num, sd_birthday, sd_phone, sd_address, sd_email, sd_date from STUDENT st, SUBJECT su where st.s_num = su.s_num order by no";
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StudentVO sVo = null;
